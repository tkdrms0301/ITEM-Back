...
public interface CategoryBrandRepository extends JpaRepository<CategoryBrand, Long> {
    @Query("select new kit.item.dto.entity.device.BrandDto (cb.id, cb.brand.id, cb.category.id, cb.brand.name) from CATEGORY_BRAND cb where cb.category.id=:categoryId")
